 ...
 *
 * License: BSD
 *   https://raw.github.com/yujinrobot/yujin_ocs/hydro/ros_common_robin_velocity_smoother/LICENSE
 **/
/*****************************************************************************
 ** Includes
 *****************************************************************************/
...
#include <ros_common_robin_velocity_smoother/paramsConfig.h>

#include <ecl/threads/thread.hpp>

#include "ros_common_robin_velocity_smoother/velocity_smoother_nodelet.hpp"
...
#define IS_ZERO_VELOCITY(a)   ((a.linear.x == 0.0) && (a.linear.y == 0.0) && (a.angular.z == 0.0))

/*****************************************************************************
** Namespaces
*****************************************************************************/

namespace ros_common_robin_velocity_smoother {
...
void VelocitySmoother::reconfigCB(ros_common_robin_velocity_smoother::paramsConfig &config, uint32_t level)
{
...
void VelocitySmoother::spin()
{
  ...
  while (! shutdown_req && ros::ok())
  {
        ...
        (target_vel.angular.z != last_cmd_vel.angular.z))
    {
      ...
      // where vx and vy acts as coordinate x and w as coordinate y; the sign of the angle from A to B determines
      // which velocity (vx and vy or w) must be overconstrained to keep the direction provided as command
      ...
      double v_inc = sqrt(vx_inc + vx_inc + vy_inc + vy_inc);
      double max_v_inc = sqrt( max_vx_inc * max_vx_inc + max_vy_inc * max_vy_inc );
      ...
      //When overconstraining linear velocities, we have to overconstrain vx and vy

      if (theta < 0)
      {
        // overconstrain linear velocity
        max_v_inc = (max_w_inc*std::abs(v_inc))/std::abs(w_inc);
        max_vx_inc = max_v_inc * max_vx_inc / max_v_inc;
        max_vy_inc = max_v_inc * max_vy_inc / max_v_inc;
...
bool VelocitySmoother::init(ros::NodeHandle& nh)
{
  ...
  dynamic_reconfigure_server = new dynamic_reconfigure::Server<ros_common_robin_velocity_smoother::paramsConfig>(nh);
...
PLUGINLIB_EXPORT_CLASS(ros_common_robin_velocity_smoother::VelocitySmootherNodelet, nodelet::Nodelet);
